...
ElemSolucao *passo(Formiga *fmg, Matriz *fluxo, Matriz *matA, FMatriz *feromonio, int atividade, Entrada *in){
    ...
    for (localidade = 0; localidade < fluxo->tam; localidade++){
        ...
        prob = probabilidade(matA, feromonio, fmg, localidade, atividade, total, in);
        if (maiorProb < prob){
            maiorProb = prob;
            maiorAtividade = atividade;
            maiorLocalidade = localidade;
            maiorValor = getElem(matA, localidade, atividade);
        }
    }
...
void caminho(Formiga *fmg, Matriz *fluxo, Matriz *matA, FMatriz *feromonio, Entrada *in){
    ...
    for (i = 0; i < fluxo->tam; i++){
        ...
        sol = passo(fmg, fluxo, matA, feromonio, atividade, in);
        addPar(fmg, sol);
    }
    for (i = 0; i < fluxo->tam; i++){
        resetMaior(fluxo, i);
...
void inicializaFeromonio(FMatriz *feromonio){
    int i, j;
    for (i = 0; i < feromonio->tam; i++){
        for (j = 0; j < feromonio->tam; j++){
            setFloat(feromonio, i, j, 0.015);
        }
    }
}
...
ElemSolucao *passoInicial(Formiga *fmg, Matriz *fluxo, Matriz *matA, FMatriz *feromonio, int atividade, Entrada *in){
    int localidade;
    int maiorAtividade, maiorLocalidade, maiorValor;
    ElemSolucao *elem;
    float total, prob, maiorProb;
...
void caminhoInicial(Formiga *fmg, Matriz *fluxo, Matriz *matA, FMatriz *feromonio, Entrada *in){
    int i, atividade;
    ElemSolucao *sol;
    for (i = 0; i < fluxo->tam; i++){
        atividade = fluxo->ref[i];
        
        sol = passoInicial(fmg, fluxo, matA, feromonio, atividade, in);
        addPar(fmg, sol);
    }
}

void inicializaFormigas(Formiga *vetorFormigas, Matriz *fluxo, Matriz *distancia, Matriz *pareada, FMatriz *feromonio, Entrada *in){
    int i, k;
    int melhorPeso = INT_MAX;
    int melhorIdFormiga;
    shuffle(fluxo, fluxo->tam);
    shuffle(distancia, distancia->tam);

    ElemSolucao *sol1;
    sol1 = malloc(fluxo->tam*sizeof(ElemSolucao));

    for (k = 0; k < in->qtdFormigas; k++){
        caminhoInicial(&vetorFormigas[k], fluxo, pareada, feromonio, in);
    }
    printf("inicio\n");    
    for (k = 0; k < in->qtdFormigas; k++){
        if (vetorFormigas[k].peso < melhorPeso){
            melhorPeso = vetorFormigas[k].peso;
            clonaVetor(sol1, vetorFormigas[k].solucao, fluxo->tam);
            melhorIdFormiga = k;
        }
    }
    atualizaFeromonio(vetorFormigas, feromonio, in);
    for (k = 0; k < in->qtdFormigas; k++){
        vetorFormigas[k] = *novaFormiga(fluxo->tam);
    }
    for (i = 0; i < fluxo->tam; i++){
        printf("%d : %d = %d\n", sol1[i].atividade, sol1[i].localidade, sol1[i].valor);
    }
    printf("\n\n");
}
